...
正因为如此，Windows 10 / 11 默认把这位房东**雪藏**起来：账户处于**禁用**状态，也不会出现在登录界面。

> 那为什么不干脆让大家都用 Administrator？
>
...
程序带有正规的**数字签名**，Windows 能够验证发布者的身份。这类相对让人放心。

**② 未知发布者（无签名 / 签名不可信）**

![无签名证书或不可信签名证书](../引用图片/使用记/介绍管理员权限与UAC/无签名证书或不可信签名证书.png)

程序没有签名，或者签名没法被验证。这不代表它一定是坏的（有些小工具确实没签名），但**必须多留个心眼**。

**③ 证书已被吊销**

![被管理员吊销证书](../引用图片/使用记/介绍管理员权限与UAC/被管理员吊销证书.png)

签名原本存在，但已经被吊销了——这通常是**明确的危险信号**。

另外还有两种常见场景：

- **系统组件自己要求提权**：Windows 自己的东西，弹窗通常偏灰，写的是 Windows 系统组件
  ![系统软件要求提权](../引用图片/使用记/介绍管理员权限与UAC/系统软件要求提权.png)
- **普通用户触发管理员操作**：如果你用的是普通账号，弹窗里会要求你输入某个管理员的账号密码
